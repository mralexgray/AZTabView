...
- (void) awakeFromNib {	[self setDefaults];	}

- (id)initWithFrame:(NSRect)frame {
    self = [super initWithFrame:frame];
    if (self) 	[self setDefaults];
    return self;
}
...
- (void) frameDidChange: (id) sender {	[self adjustTabLayerScrollAnimated:NO];	}

- (void) adjustTabLayerScrollAnimated: (BOOL) animated {

	if (currentSelectedTab == nil) 	return;
    ...
    if(tabsLayer.visibleRect.size.width < ([self bounds].size.width - ([self lastTab].frame.size.width / 2.0)) && tabsLayer.visibleRect.origin.x > 0) {
        ...
        if (newTabXPosition < 0) 	newTabXPosition = 0;
...
- (CALayer *) tabsLayer {
	...
	tabsLayer.constraints   = @[	AZConstRelSuper(kCAConstraintHeight),
								AZConstRelSuper(kCAConstraintMinY),
								AZConstRelSuper(kCAConstraintMaxY)	];
	NSMutableDictionary *actions= [tabsLayer actions].mutableCopy;
...
-(void)scrollWheel:(NSEvent *)theEvent {
		//    NSLog(@"Scroll Event: %@", theEvent);
	if ( abs(theEvent.deltaX+theEvent.deltaY)> 6) {
		if (theEvent.deltaX < 0)	[self selectPreviousTab:nil];
		else 						[self selectNextTab:nil];
	}
}

- (void)mouseDown: (NSEvent *) theEvent {
		// Getting clicked point.
    mouseDownPoint = [self.layer convertPoint: [self convertPoint:theEvent.locationInWindow fromView:nil]toLayer:tabsLayer];
	// Checking if a tab was clicked.
    CALayer *clickedLayer = [tabsLayer hitTest:mouseDownPoint];
	if (clickedLayer &&  clickedLayer != tabsLayer ) {
        canDragTab = NO;
        BOOL shouldSelectTab = YES;			// Asking delegate if the tab can be selected.
...
- (void)mouseDragged: (NSEvent *) theEvent {		// convert to local coordinate system
    NSPoint mousePointInView = [self.layer convertPoint: [self convertPoint:theEvent.locationInWindow fromView:nil]toLayer:tabsLayer];
...
- (void) addTabAtIndex: (int) index withRepresentedObject: (id) representedObject {
    ...
    if ([newtab respondsToSelector:@selector(setViewDelegate:)]) {
        [newtab setViewDelegate:self];
    }
